#!/usr/bin/env python3
"""Docker TUI — list, inspect, edit and redeploy containers."""
...
def _container_snapshot(c) -> dict[str, Any]:
    """Extract everything needed to recreate a container."""
    attrs = c.attrs
    ...
    for cport, bindings in (hcfg.get("PortBindings") or {}).items():
        if bindings:
            ports[cport] = [{"HostIp": b.get("HostIp", ""), "HostPort": b.get("HostPort", "")} for b in bindings]
        else:
            ports[cport] = None
...
def _redeploy(snap: dict[str, Any]) -> None:
    ...
    try:
        ...
        was_running = True  # new container: start it
    ...
    if snap["hostname"]:
        kwargs["hostname"] = snap["hostname"]
    if snap["command"]:
        kwargs["command"] = snap["command"]
    if snap["entrypoint"]:
        kwargs["entrypoint"] = snap["entrypoint"]
    if snap["ports"]:
        kwargs["ports"] = snap["ports"]
...
class WizardScreen(ModalScreen[dict | None]):
    ...
    def __init__(self, snap: dict[str, Any] | None = None) -> None:
        super().__init__()
        self._snap = snap  # None = new container
            ...
            with TabbedContent(id="wizard-tabs"):
                ...
                with TabPane("Basic", id="tab-basic"):
                    with ScrollableContainer():
                        yield Label("Name" + ("" if is_new else "  (fixed — create new container to rename)"), classes="field-lbl")
                        yield Input(
                            value=snap.get("name", ""),
                        ...
                        yield Select(
                            options=[(p, p) for p in RESTART_POLICIES],
                            value=rp,
                            id="sel-restart",
                        )
...
class VolumePickerScreen(ModalScreen[str | None]):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="vpick-outer"):
            ...
            yield ListView(
                *[ListItem(Label(v), name=v) for v in self._volumes],
                id="vpick-list",
            )
...
COLUMNS = ("Name", "Status", "Image", "Networks / IPs")


class MainScreen(Screen):
    BINDINGS = [
        Binding("r", "refresh", "Refresh"),
        Binding("n", "new_container", "New"),
    ...
    def action_delete_selected(self) -> None:
        ...
        if not row:
            return
        msg = (
            f"Delete [bold]{row['name']}[/bold]?\n\n"
            "This will [red]permanently remove[/red] the container.\n"
            "Volumes and images are left intact."
        )
        self.app.push_screen(ConfirmScreen(msg), lambda ok: self._exec_delete(ok, row))

    def _exec_delete(self, ok: bool, row: dict) -> None:
        if not ok:
            return
    ...
    def action_new_container(self) -> None:
        self.app.push_screen(WizardScreen(snap=None), self._after_new)

    def _after_new(self, snap: dict | None) -> None:
        if snap is None:
            return
    ...
    def _load_containers(self) -> None:
        try:
            ...
            self.app.call_from_thread(self.notify, f"Docker error: {exc}", severity="error", timeout=8)
            return
    ...
    def _render_containers(self) -> None:
        ...
        if self._first_load:
            label = "all" if self._show_stopped else "running only"
            self.notify(f"Loaded {len(visible)} containers ({label})", timeout=2)
            self._first_load = False

    def do_redeploy(self, snap: dict[str, Any]) -> None:
        msg = (
            f"Redeploy [bold]{snap['name']}[/bold]?\n\n"
...
/* ── modal alignment ── */
WizardScreen, DetailScreen, ConfirmScreen { align: center middle; }
